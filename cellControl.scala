...
object CellControl {
  def apply(nodez: Array[Node]): CellControl = new CellControl(nodez) {
    ...
    override def isSM(f: Int)     : Boolean = f > 25
    override def isInput(f: Int)  : Boolean = f <= 4
    override def isOutput(f: Int) : Boolean = f == 25
    override def isFlush(f: Int)  : Boolean = f == 34
